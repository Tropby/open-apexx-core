...
CODE;
	...
	function mshow()
	{
		...
		foreach ($apx->coremodules as $modulename)
		{
			...
			//Aktualisieren
			if (file_exists(BASEDIR . $apx->path()->getmodulepath($modulename) . 'setup.php'))
			{
	...
	function menable()
	{
		...
		if (isset($_REQUEST['multiid']) && is_array($_REQUEST['multiid']))
		{
			...
			else
			{
				...
				foreach ($modules as $id) logit('MAIN_MENABLE', $id);
	...
	function mdisable()
	{
		...
		if (isset($_REQUEST['multiid']) && is_array($_REQUEST['multiid']))
		{
			...
			else
			{
				...
				foreach ($modules as $id) logit('MAIN_MDISABLE', $id);
		...
		if (isset($_REQUEST['multiid']) && is_array($_REQUEST['multiid']))
		{
			...
			else
			{
				...
				foreach ($_REQUEST['multiid'] as $module)
				{
					...
					if (!$this->checkRequirement($regmods[$module], $regmods)) continue; //Anforderungen nicht erfüllt
				...
				{
					$mp = $apx->path()->getAbsoliteModulePath($module);

					/**
					 * Old style setup
					 * @deprecated
					 */
					if (file_exists($mp . "setup.php"))
					{
						require(BASEDIR . $apx->path()->getmodulepath($module) . 'setup.php');
					...
					{
						$m = "\\Modules\\" . $_REQUEST['module'] . "\\Setup";
						/**
						 * @var \Setup
						 */
						$setup = new $m();
	...
	function muninstall()
	{
		...
		if (isset($_REQUEST['multiid']) && is_array($_REQUEST['multiid']))
		{
			...
			else
			{
				...
				{
					/**
					 * @deprecated old style setup will be deleted
					 */
