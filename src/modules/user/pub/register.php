...
if ( isset( $_POST['send'] ) ) {
	...
	if ( !$set['user']['mailmultiacc'] ) list($check2)=$db->first("SELECT email FROM ".PRE."_user WHERE LOWER(email)='".addslashes(strtolower($_POST['email1']))."' LIMIT 1");
	...
	elseif ( $set['user']['userminlen'] && strlen($_POST['username'])<$set['user']['userminlen'] ) message($apx->lang->get('MSG_USERLENGTH',array('LENGTH'=>$set['user']['userminlen'])),'javascript:history.back()');
	elseif ( $set['user']['pwdminlen'] && strlen($_POST['pwd1'])<$set['user']['pwdminlen'] ) message($apx->lang->get('MSG_PWDLENGTH',array('LENGTH'=>$set['user']['pwdminlen'])),'javascript:history.back()');
	...
	elseif ( !$set['user']['mailmultiacc'] && $check2 ) message($apx->lang->get('MSG_MAILEXISTS'),'javascript:history.back()');
	else {
		...
		if ( $set['user']['useractivation']==2 ) $_POST['reg_key']='BYADMIN';
		elseif ( $set['user']['useractivation']==3 ) $_POST['reg_key']=random_string();
