...
class AddBuddy extends \PublicAction
{
	public function execute()
	{
		...
		{
			$authid = md5($_REQUEST['id'] . $apx->config('main')['crypt'] . $user->info['userid']);
			if ($_REQUEST['id'] == $user->info['userid']) message($apx->lang->get('MSG_NOTSELFCONFIRM'));
			elseif ($authid != $_REQUEST['verify']) message($apx->lang->get('MSG_INVALIDKEY'));
			else
			{
				...
				if (!$inlist)
				{
					...
					$text = $apx->lang->get('MAIL_FINISHED_TEXT', array(
						'USERNAME' => $userinfo['username'],
						'SENDER' => $user->info['username'],
						'WEBSITE' => $apx->config('main')['websitename']
					));
		...
		elseif ($_POST['send'])
		{
			...
			//User suchen
			$userinfo = $db->first("SELECT userid,username,email,pub_mailpm,pub_poppm FROM " . PRE . "_user WHERE userid='" . $_REQUEST['id'] . "' LIMIT 1");
			if (!$userinfo['userid']) message($apx->lang->get('MSG_NOTEXISTS'), 'back');
			elseif ($userinfo['userid'] == $user->info['userid']) message($apx->lang->get('MSG_NOTSELF'), 'back');
			else
			{

				$authid = md5($user->info['userid'] . $apx->config('main')['crypt'] . $_REQUEST['id']);
				...
				$text = $apx->lang->get('PM_TEXT', array(
					'USERNAME' => $userinfo['username'],
					'SENDER' => $user->info['username'],
					'WEBSITE' => $apx->config('main')['websitename'],
					'URL' => $authlink
		...
		else
		{
			...
			{
				tmessage('addbuddy', array('ID' => $_REQUEST['id']));
			}
		}
	}
}
