...
class Blog extends \PublicAction
{
	public function execute()
	{
		...
		if (!$apx->config('user')['blog']) die('function disabled!');
		...
		if ($_REQUEST['blogid'])
		{
			$apx->headline($apx->lang->get('HEADLINE_BLOG'), str_replace('&', '&amp;', $_SERVER['REQUEST_URI']));
			$apx->titlebar($apx->lang->get('HEADLINE_BLOG'));
		...
		else
		{
			...
			pages(
				...
				$count,
				$apx->config('user')['blog_epp']
			);

			//Einträge auslesen
			$data = $db->fetch("SELECT * FROM " . PRE . "_user_blog WHERE userid='" . $_REQUEST['id'] . "' ORDER BY time DESC" . getlimit($apx->config('user')['blog_epp']));
			if (count($data))
			{
				foreach ($data as $res)
				{
