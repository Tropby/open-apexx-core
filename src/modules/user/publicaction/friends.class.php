...
class Friends extends \PublicAction
{
	public function execute()
	{
		...
		$apx->headline($apx->lang->get('HEADLINE_FRIENDS'), str_replace('&', '&amp;', $_SERVER['REQUEST_URI']));
		$apx->titlebar($apx->lang->get('HEADLINE_FRIENDS'));
		...
		pages($pagelink, $count, $apx->config('user')['friendsepp']);
		...
		if (count($buddies))
		{
			$data = $db->fetch("SELECT userid,username,groupid,realname,gender,city,plz,country,city,lastactive,pub_invisible,avatar,avatar_title,custom1,custom2,custom3,custom4,custom5,custom6,custom7,custom8,custom9,custom10 FROM " . PRE . "_user WHERE userid IN (" . implode(',', $buddies) . ") ORDER BY username ASC" . getlimit($apx->config('user')['friendsepp']));
			if (count($data))
			{
				$i = 0;
				foreach ($data as $res)
				{
					...
					$userdata[$i]['ONLINE'] = iif(!$res['pub_invisible'] && ($res['lastactive'] + $apx->config('user')['timeout'] * 60) >= time(), 1, 0);
