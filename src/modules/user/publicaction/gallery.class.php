...
class Gallery extends \PublicAction
{
	public function execute()
	{
		...
		if ($_REQUEST['galid'])
		{
			...
			if (!$gallery['id']) die('access denied!');
			...
			pages(
				...
				$count,
				$apx->config('user')['gallery_epp']
			);

			//Einträge auslesen
			$data = $db->fetch("SELECT * FROM " . PRE . "_user_pictures WHERE galid='" . $galid . "' ORDER BY addtime DESC" . getlimit($apx->config('user')['gallery_epp']));
			if (count($data))
			{
				foreach ($data as $res)
				{
