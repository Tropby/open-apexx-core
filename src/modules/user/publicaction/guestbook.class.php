...
class Guestbook extends \PublicAction
{
	public function execute()
	{
		...
		if (!$apx->config('user')['guestbook']) die('function disabled!');
		...
		if ($_REQUEST['add'])
		{
			...
			elseif (!$_POST['text'] || ($apx->config('user')['guestbook_req_title'] && !$_POST['title'])) message('back');
			...
			elseif ($apx->config('user')['guestbook_entrymaxlen'] && strlen($_POST['text']) > $apx->config('user')['guestbook_entrymaxlen']) message($apx->lang->get('MSG_TOOLONG'), 'back');
			elseif (($spam + $apx->config('user')['guestbook_spamprot'] * 60) > time()) message($apx->lang->get('MSG_BLOCKSPAM', array('SEC' => ($spam + $apx->config('user')['guestbook_spamprot'] * 60) - time())), 'back');
			else
			{
		...
		else
		{
			...
			pages(
				...
				$count,
				$apx->config('user')['guestbook_epp']
			);


			//Einträge auslesen
			$tabledata = array();
			$data = $db->fetch("SELECT * FROM " . PRE . "_user_guestbook WHERE owner='" . $_REQUEST['id'] . "' ORDER BY time DESC " . getlimit($apx->config('user')['guestbook_epp']));
			$entrynumber = $count - ($_REQUEST['p'] - 1) * $apx->config('user')['guestbook_epp'];
			...
			if (count($data))
			{
				foreach ($data as $res)
				{
					...
					$tabledata[$i]['ONLINE'] = iif(!$res['pub_invisible'] && ($res['lastactive'] + $apx->config('user')['timeout'] * 60) >= time(), 1, 0);
					$tabledata[$i]['ISONLINE'] = $tabledata[$i]['ONLINE'];

					//Text
					$text = $res['text'];
					if ($apx->config('user')['guestbook_breakline']) $text = wordwrap($text, $apx->config('user')['guestbook_breakline'], "\n", 1);
					if ($apx->config('user')['guestbook_badwords']) $text = badwords($text);
					$text = replace($text, 1);
					if ($apx->config('user')['guestbook_allowsmilies']) $text = dbsmilies($text);
					if ($apx->config('user')['guestbook_allowcode']) $text = dbcodes($text);

					//Titel
					$title = $res['title'];
					if ($apx->config('user')['guestbook_breakline']) $title = wordwrap($title, $apx->config('user')['guestbook_breakline'], "\n", 1);
					if ($apx->config('user')['guestbook_badwords']) $title = badwords($title);
					$title = replace($title);
					...
					//Admin-Links
					if ($apx->config('user')['guestbook_useradmin'] && $_REQUEST['id'] == $user->info['userid'])
					{
