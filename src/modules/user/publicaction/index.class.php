...
class Index extends \PublicAction
{
	public function execute()
	{
		...
		if ($apx->is_module('products') && $apx->config('products')['collection'])
		{
		...
		//PMs
		list($totalpms) = $apx->db()->first("SELECT count(id) FROM " . PRE . "_user_pms WHERE ( touser='" . $user->info['userid'] . "' AND del_to='0' )");
		list($newpms) = $apx->db()->first("SELECT count(id) FROM " . PRE . "_user_pms WHERE ( touser='" . $user->info['userid'] . "' AND del_to='0' AND isread='0' )");
		$apx->tmpl->assign('TOTALPMS', $totalpms);
		$apx->tmpl->assign('NEWPMS', $newpms);
		if ($apx->config('user')['guestbook'])
		{
			list($newgb) = $apx->db()->first("SELECT count(id) FROM " . PRE . "_user_guestbook WHERE owner='" . $user->info['userid'] . "' AND userid!='" . $user->info['userid'] . "' AND time>'" . $user->info['lastonline'] . "'");
