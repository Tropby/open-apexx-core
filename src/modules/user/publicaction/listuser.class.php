...
class ListUser extends \PublicAction
{
	public function execute()
	{
		...
		if ($_REQUEST['letter'])
		{
			if ($_REQUEST['letter'] == 'spchar') $where = "AND username NOT REGEXP(\"^[a-zA-Z]\")";
			else $where = "AND username LIKE '" . addslashes($_REQUEST['letter']) . "%'";
		}
		...
		$data = $db->fetch("SELECT " . $fields . " FROM " . PRE . "_user WHERE ( active='1' " . $where . " ) " . getorder($orderdef) . " " . getlimit($apx->config('user')['userlistepp']));
		if (count($data))
		{
			$i = 0;
			foreach ($data as $res)
			{
				...
				if ($user->info['userid'] ?? 0)
				{
					...
					if (in_array('USER.LINK_BUDDY', $parse) && $res["userid"] != $user->info['userid'] && !$user->is_buddy($res['userid']))
					{
