...
class MyGallery extends \PublicAction
{
	public function execute()
	{
		...
		if (!$apx->config('user')['gallery']) die('function disabled!');
		...
		if ($_REQUEST['galid'])
		{
			...
			if ($_REQUEST['do'] == 'add')
			{
				if ($_POST['send'])
				{
					...
					for ($i = 1; $i <= 3; $i++)
					{
						...
						if (!$mm->uploadfile($_FILES['upload' . $i], 'temp', $mm->getfile($_FILES['upload' . $i]['tmp_name']))) continue;
					...
					//Bilderzahl auslesen
					$piccount = 0;
					if ($apx->config('user')['gallery_maxpics'])
					{
					...
					if (!count($files)) message('back');
					elseif ($apx->config('user')['gallery_maxpics'] && $piccount + count($files) > $apx->config('user')['gallery_maxpics']) message($apx->lang->get('MSG_PICLIMIT', array('LIMIT' => $apx->config('user')['gallery_maxpics'])), 'back');
					else
					{
						...
						foreach ($files as $file)
						{
							...
							//////// THUMBNAIL
							$thumbnail = $img->resize($picture, $apx->config('user')['gallery_thumbwidth'], $apx->config('user')['gallery_thumbheight'], $apx->config('user')['gallery_quality_resize'], $apx->config('user')['gallery_thumb_fit']);
							...
							//Skalieren
							if ($picture !== false && !$file['noresize'] && $apx->config('user')['gallery_picwidth'] && $apx->config('user')['gallery_picheight'])
							{
								$scaled = $img->resize(
									$picture,
									$apx->config('user')['gallery_picwidth'],
									$apx->config('user')['gallery_picheight'],
									$apx->config('user')['gallery_quality_resize'],
									0
								);
				...
				else
				{
					...
					if ($apx->config('user')['gallery_maxpics'])
					{
						...
						$space = $piccount . '/' . $apx->config('user')['gallery_maxpics'];
						$percent = (round(($apx->config('user')['gallery_maxpics'] - $piccount) / iif($apx->config('user')['gallery_maxpics'] == 0, 1, $apx->config('user')['gallery_maxpics']), 2) * 100) . '%';
						$width = (100 - $percent) . '%';
					...
					//Limit erreicht
					if ($apx->config('user')['gallery_maxpics'] && $piccount >= $apx->config('user')['gallery_maxpics'])
					{
						message($apx->lang->get('MSG_PICLIMITREACHED', array('LIMIT' => $apx->config('user')['gallery_maxpics'])), 'back');
			...
			elseif ($_REQUEST['do'] == 'del')
			{
				...
				else tmessage('delgallerypic', array('ID' => $_REQUEST['id']));
				require('lib/_end.php');
			}
		...
		else
		{
			...
			elseif ($_REQUEST['do'] == 'del')
			{
				...
				else tmessage('delgallery', array('ID' => $_REQUEST['id']));
				require('lib/_end.php');
			}
