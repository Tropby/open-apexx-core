...
class MyProfile extends \PublicAction
{
	public function execute()
	{
		...
		$apx->headline($apx->lang->get('HEADLINE_MYPROFILE'), str_replace('&', '&amp;', $_SERVER['REQUEST_URI']));
		$apx->titlebar($apx->lang->get('HEADLINE_MYPROFILE'));
		...
		if ($_POST['send'] ?? 0)
		{
			...
			{
				require_once(BASEDIR . getmodulepath('user') . 'class/class.identycard.php');
				...
				elseif (strlen($_POST['identcode'][0]) != 10 || strlen($_POST['identcode'][1]) != 7 || strlen($_POST['identcode'][2]) != 7)
				{
					$identError = $apx->lang->get('MSG_IDENT_INCOMPLETE');
				}
				...
				{
					$identError = $apx->lang->get('MSG_IDENT_TOOYOUNG', array('AGE' => $identCard->getAge()));
				}
				else
				{
					$identConfirmed = true;
			...
			if ((($_POST['pwd1'] || $_POST['pwd2']) && (!$_POST['pwd1'] || !$_POST['pwd2'])) || !$_POST['email']) message('back');
			elseif ($_POST['pwd1'] != $_POST['pwd2']) message($apx->lang->get('MSG_PWNOMATCH'), 'javascript:history.back()');
			elseif ($_POST['pwd1'] && $set['user']['pwdminlen'] && strlen($_POST['pwd1']) < $set['user']['pwdminlen']) message($apx->lang->get('MSG_PWDLENGTH', array('LENGTH' => $set['user']['pwdminlen'])), 'javascript:history.back()');
			elseif ($identError) message($identError, 'javascript:history.back()');
			elseif (!checkmail($_POST['email'])) message($apx->lang->get('MSG_NOMAIL'), 'javascript:history.back()');
			else
			{
				...
				if ($user->info['email'] != $_POST['email'] && $set['user']['useractivation'] == 3 && $set['user']['reactivate'])
				{
				...
				else
				{
					...
					if ($_POST['pwd1']) message($apx->lang->get('MSG_OK_NEWPWD'), mklink('user.php?action=logout', 'user,logout.html'));
					else message($apx->lang->get('MSG_OK'), mklink('user.php', 'user.html'));
				}
			}
		}
		else
		{
			...
			if (count($apx->config('main')['smilies']))
			{
				foreach ($apx->config('main')['smilies'] as $res)
				{
					++$i;
