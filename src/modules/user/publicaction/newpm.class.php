...
class NewPM extends \PublicAction
{
	public function execute()
	{
		...
		if ($pmcount_own >= $apx->config('user')['maxpmcount'])
		{
		...
		if ($_POST['send'])
		{
			...
			if ($touser)
			{
				...
				if ($pmcount_rec >= $apx->config('user')['maxpmcount'])
				{
			...
			if (!$_POST['touser'] || !$_POST['subject'] || !$_POST['text']) message('back');
			elseif (!$touser) message($apx->lang->get('MSG_NOTEXISTS'), 'javascript:history.back()');
			elseif ($touser == $user->info['userid']) message($apx->lang->get('MSG_SELF'), 'javascript:history.back()');
			...
			else
			{
				...
				{
					$text = $_POST['text'];
					if ($apx->config('user')['pm_badwords']) $text = badwords($text);
					$text = replace($text, 1);
					if ($apx->config('user')['pm_allowsmilies']) $text = dbsmilies($text);
					if ($apx->config('user')['pm_allowcode']) $text = dbcodes($text);
