...
class Online extends \PublicAction
{
	public function execute()
	{
		...
		list($count['users']) = $db->first("SELECT count(*) FROM " . PRE . "_user WHERE lastactive>=" . (time() - $apx->config('user')['timeout'] * 60));
		list($count['inv']) = $db->first("SELECT count(*) FROM " . PRE . "_user WHERE lastactive>=" . (time() - $apx->config('user')['timeout'] * 60) . " AND pub_invisible=1");
		if ($apx->config('user')['onlinelist'])
		{
		...
		$data = $db->fetch("SELECT b.userid,b.username,b.email,b.pub_hidemail,b.groupid,b.realname,b.gender,b.city,b.plz,b.country,b.city,b.lastactive,b.pub_invisible,b.avatar,b.avatar_title FROM " . PRE . "_user AS b WHERE ( b.lastactive>=" . (time() - $apx->config('user')['timeout'] * 60) . " AND b.pub_invisible='0' ) ORDER BY b.username ASC");
		$tabledata = array();
		if (count($data))
		{
			$i = 0;
			foreach ($data as $res)
			{
				...
				$tabledata[$i]['REGTIME'] = $res['reg_time'] ?? 0;
				$tabledata[$i]['REGDAYS'] = floor((time() - ($res['reg_time'] ?? 0)) / (24 * 3600));
				$tabledata[$i]['LASTACTIVE'] = $res['lastactive'] ?? 0;
				...
				if ($user->info['userid'] ?? 0)
				{
					...
					if ((in_array('USER.LINK_BUDDY', $parse) || in_array('USER.LINK_ADDBUDDY', $parse)) && $res["userid"] != $user->info['userid'] && !$user->is_buddy($res['userid']))
					{
