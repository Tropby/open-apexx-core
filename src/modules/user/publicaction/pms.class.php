...
class PMS extends \PublicAction
{
	public function execute()
	{
		...
		if ($_REQUEST['dir'] != 'in' && $_REQUEST['dir'] != 'out') $_REQUEST['dir'] = 'in';

		//PM Status verändern
		if ($_REQUEST['read']) $db->query("UPDATE " . PRE . "_user_pms SET isread='1' WHERE ( id='" . $_REQUEST['read'] . "' AND touser='" . $user->info['userid'] . "' ) LIMIT 1");
		elseif ($_REQUEST['unread']) $db->query("UPDATE " . PRE . "_user_pms SET isread='0' WHERE ( id='" . $_REQUEST['unread'] . "' AND touser='" . $user->info['userid'] . "' ) LIMIT 1");
		...
		if (is_array($_POST['multi']))
		{
			...
			if (is_array($dcache))
			{
				if ($_REQUEST['dir'] == 'out') $db->query("UPDATE " . PRE . "_user_pms SET del_from='1' WHERE ( id IN (" . implode(",", $dcache) . ") AND fromuser='" . $user->info['userid'] . "' )");
				else $db->query("UPDATE " . PRE . "_user_pms SET del_to='1' WHERE ( id IN (" . implode(",", $dcache) . ") AND touser='" . $user->info['userid'] . "' )");
			}
		}
		...
		$space = $pmcount . '/' . $apx->config('user')['maxpmcount'];
		$percent = round(max(round(($apx->config('user')['maxpmcount'] - $pmcount) / iif($apx->config('user')['maxpmcount'] == 0, 1, $apx->config('user')['maxpmcount']), 2) * 100, 0));
		...
		//Orderby
		$orderdef[0] = 'time';
		$orderdef['subject'] = array('a.subject', 'ASC', 'SORT_SUBJECT');
		$orderdef['username'] = array('b.username', 'ASC', 'SORT_USERNAME');
		$orderdef['time'] = array('a.time', 'DESC', 'SORT_TIME');

		//Auflistung
		if ($_REQUEST['dir'] == 'out') $data = $db->fetch("SELECT a.id,a.subject,a.time,a.isread,b.userid,b.username FROM " . PRE . "_user_pms AS a LEFT JOIN " . PRE . "_user AS b ON a.touser=b.userid WHERE ( fromuser='" . $user->info['userid'] . "' AND del_from='0' )" . getorder($orderdef));
		else $data = $db->fetch("SELECT a.id,a.subject,a.time,a.isread,b.userid,b.username FROM " . PRE . "_user_pms AS a LEFT JOIN " . PRE . "_user AS b ON a.fromuser=b.userid WHERE ( touser='" . $user->info['userid'] . "' AND del_to='0' ) " . getorder($orderdef));
		if (count($data))
		{
			$i=0;
			foreach ($data as $res)
