...
class Profile extends \PublicAction
{
	public function execute()
	{
		...
		$apx->tmpl->assign('IS_ONLINE', iif(!$res['pub_invisible'] && ($res['lastactive'] + $apx->config('user')['timeout'] * 60) >= time(), 1, 0));
		...
		for ($i = 1; $i <= 10; $i++)
		{
			$apx->tmpl->assign('CUSTOM' . $i . '_NAME', replace($apx->config('user')['cusfield_names'][($i - 1)]));
		...
		$link_sendmail = iif(($user->info['userid'] || $apx->config('user')['sendmail_guests']) && $user->info['userid'] != $_REQUEST['id'], mklink(
