...
class Report extends \PublicAction
{
	public function execute()
	{
		...
		if (!$_REQUEST['contentid']) die('missing ContentID!');
		...
		if ($_POST['send'])
		{
			...
			else
			{
				...
				$id = (int)$id;
				if (!in_array($content, array('blog', 'blogentry', 'gallery', 'profile'))) die('invalid content-name!');
				...
				//eMail senden
				if ($apx->config('user')['reportmail'])
				{
					$input['URL'] = HTTP_HOST . $url;
					$input['REASON'] = $_POST['text'];
					sendmail($apx->config('user')['reportmail'], 'REPORT', $input);
				}
