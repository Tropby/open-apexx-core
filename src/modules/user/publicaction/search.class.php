...
class Search extends \PublicAction
{
	public function execute()
	{
		...
		");
			...
			");
					...
					if (count($data) > 1 && $_POST['city'])
					{
						...
						{
							if (!user_city_match($res['name'], $_POST['city'])) unset($data[$key]);
						}
						$locids = get_ids($data, 'id');
					}
				}
		...
		");
			...
			if ($_POST['item'])
			{
				...
				for ($i = 1; $i <= 10; $i++)
				{
					if ($apx->config('user')['cusfield_names'][($i - 1)])
					{
			...
			if ($_POST['gender'])
			{
				if ($_POST['gender'] == 1) $where .= " AND gender=1 ";
				elseif ($_POST['gender'] == 2) $where .= " AND gender=2 ";
			}
		...
		$where .= " AND userid IN (".implode(',', $onlineIds).") ";*/
				$where .= " AND lastactive>=" . (time() - $apx->config('user')['timeout'] * 60) . " AND pub_invisible=0";
			}
			...
			else
			{
				...
				if ($apx->config('user')['listactiveonly'])
				{
					$where .= " AND reg_key='' ";
				}
		...
		if (isset($_REQUEST['searchid']))
		{
			...
			pages($pagelink, $count, $apx->config('user')['userlistepp']);

			//Orderby
			$orderdef[0] = 'username';
			$orderdef['regdate'] = array('reg_time', 'DESC');
			$orderdef['username'] = array('username', 'ASC');
			if ($apx->is_module('forum')) $orderdef['forumposts'] = array('forum_posts', 'DESC');

			if ($apx->is_module('forum'))
			{
				$data = $db->fetch("SELECT userid,username,email,reg_time,pub_hidemail,groupid,realname,gender,city,plz,country,city,lastactive,avatar,avatar_title,forum_posts,custom1,custom2,custom3,custom4,custom5,custom6,custom7,custom8,custom9,custom10 FROM " . PRE . "_user WHERE ( active='1' AND userid IN (" . implode(',', $results) . ") ) " . getorder($orderdef) . " " . getlimit($apx->config('user')['userlistepp']));
			}
			else
			{
				$data = $db->fetch("SELECT userid,username,email,reg_time,pub_hidemail,groupid,realname,gender,city,plz,country,city,lastactive,avatar,avatar_title,custom1,custom2,custom3,custom4,custom5,custom6,custom7,custom8,custom9,custom10 FROM " . PRE . "_user WHERE ( active='1' AND userid IN (" . implode(',', $results) . ") ) " . getorder($orderdef) . " " . getlimit($apx->config('user')['userlistepp']));
			}
			if (count($data))
			{
				foreach ($data as $res)
				{
					...
					for ($ii = 1; $ii <= 10; $ii++)
					{
						$tabledata[$i]['CUSTOM' . $ii . '_NAME'] = $apx->config('user')['cusfield_names'][($ii - 1)];
					...
					$tabledata[$i]['LINK_SENDEMAIL'] = iif($user->info['userid'] || $apx->config('user')['sendmail_guests'], mklink(
