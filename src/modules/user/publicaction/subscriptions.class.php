...
class Subscriptions extends \PublicAction
{
	public function execute()
	{
		...
		if (count($data)) foreach ($data as $res) $subsrcinfo_forums[$res['source']] = $res;
		...
		if (count($data)) foreach ($data as $res) $subsrcinfo_threads[$res['source']] = $res;
		...
		if (count($subscr_forums))
		{
			...
			foreach ($subscr_forums as $forumid)
			{
				...
				if ($notification == 'none') $notify = $apx->lang->get('SUBSCRIPTION_NONE');
				elseif ($notification == 'instant') $notify = $apx->lang->get('SUBSCRIPTION_INSTANT');
				elseif ($notification == 'daily') $notify = $apx->lang->get('SUBSCRIPTION_DAILY');
				elseif ($notification == 'weekly') $notify = $apx->lang->get('SUBSCRIPTION_WEEKLY');
				...
				if (correct_forum_password($res))
				{
					...
					if ($res['lastposttime']) $thread = $db->first("SELECT threadid,prefix,title FROM " . PRE . "_forum_threads WHERE ( del=0 AND moved=0 AND forumid='" . $res['forumid'] . "' ) ORDER BY lastposttime DESC LIMIT 1");
