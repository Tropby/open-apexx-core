...
class UserMap extends \PublicAction
{
	public function execute()
	{
		...
		$topleft_x = $apx->config('user')['usermap_topleft_x'];
		$topleft_y = $apx->config('user')['usermap_topleft_y'];
		$bottomright_x = $apx->config('user')['usermap_bottomright_x'];
		$bottomright_y = $apx->config('user')['usermap_bottomright_y'];
		$mapwidth = $apx->config('user')['usermap_width'];
		$mapheight = $apx->config('user')['usermap_height'];
		...
		if (count($data))
		{
			foreach ($data as $res)
			{
				...
				if (!isset($location[$locationid])) $location[$locationid] = array();
		...
		if (count($location))
		{
			...
			if (count($position))
			{
				...
				{
					if (!isset($position[$lockey])) continue;
					$posinfo = $position[$lockey];
					$posx = $posinfo['l'];
					$posy = $posinfo['b'];

					if (!($posx >= $topleft_x && $posx <= $bottomright_x && $posy <= $topleft_y && $posy >= $bottomright_y)) continue;
					...
					if ($usercount > 10) $sub = 3;
					elseif ($usercount > 5) $sub = 2;
					else $sub = 1;
					$posleft -= $sub;
					$postop -= $sub;
