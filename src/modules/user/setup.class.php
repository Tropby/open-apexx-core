...
class Setup extends \Setup
{
	...
	function update(\Apexx $apx, int $installed_version)
	{
		switch ($installed_version)
		{
			...
			case 110: //zu 1.1.1
				...
				$data = $apx->db()->fetch("SELECT userid,city,plz,country,buddies,birthday FROM " . PRE . "_user");
				if (count($data))
				{
					foreach ($data as $res)
					{
						...
						if (is_array($buddies))
						{
							...
							{
								$apx->db()->query("INSERT INTO " . PRE . "_user_friends VALUES ('" . $res['userid'] . "','" . $id . "')");
							}
						}
						...
						//Update Tabelle
						$apx->db()->query("UPDATE " . PRE . "_user SET locid='" . $locid . "',birthday='" . $birthday . "' WHERE userid='" . $res['userid'] . "' LIMIT 1");
					}
				}
				$apx->db()->query("UPDATE " . PRE . "_user SET birthday='' WHERE birthday='00-00'");
				$apx->db()->query("ALTER TABLE " . PRE . "_user DROP `buddies` ;");
			...
			case 113: //zu 1.1.4
				list($check) = $apx->db()->first("SELECT module FROM " . PRE . "_config WHERE module='user' AND varname='avatar_resize' LIMIT 1");
				if (!$check)
				{
					$apx->db()->query("INSERT INTO " . PRE . "_config VALUES ('user', 'avatar_resize', 'switch', '', '0', '0', '950')");
				}
			...
			case 121: //zu 1.2.2
				...
				$data = $apx->db()->fetch("SELECT userid,city,plz,country FROM " . PRE . "_user");
				if (count($data))
				{
					foreach ($data as $res)
					{
						...
						//Update Tabelle
						$apx->db()->query("UPDATE " . PRE . "_user SET locid='" . $locid . "' WHERE userid='" . $res['userid'] . "' LIMIT 1");
					}
				}
	...
	{
		throw "Can not uninstall user module!";
	}
